...
class UnplanCameraPage extends StatefulWidget {
  const UnplanCameraPage({super.key});

  @override
  State<UnplanCameraPage> createState() => _UnplanCameraPageState();
}

class _UnplanCameraPageState extends State<UnplanCameraPage>
  ...
  // throttle lokasi biar ga spam (hindari freeze)
  ...
  Future<void> _takePicture() async {
    ...
    try {
      ...
      // TODO: upload file.path + _lat/_lng + _address + tanggal (_todayText)
    } catch (e) {
      if (!mounted) return;
      setState(() => _isTaking = false);
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _openMaps() async {
    ...
    final geoUri = Uri.parse('geo:$lat,$lng?q=$lat,$lng(Unplan Kunjungan)');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Unplan Kamera'),
