...
import 'tambah_customer_page.dart';
import 'edit_customer_page.dart';
import 'harga_produk_customer_page.dart';
...
class _CustomerPageState extends State<CustomerPage> {
  ...
  final List<String> _areas = ['Semua Area', 'Jakarta', 'Bogor', 'Depok', 'Tangerang', 'Bekasi','Sleman'];
  ...
  final List<CustomerItem> _items = [
    ...
    CustomerItem(
      ...
      name: 'PMI Sleman',
      phone: '(0274) 868900',
      email: 'pmi@sleman.com',
      address: 'Jl. Dr. Radjimin, Sucen, Triharjo, Kec. Sleman, Kabupaten Sleman',
      type: 'PMI',
      area: 'Sleman',
    ),
  ];
  ...
  List<CustomerItem> get _filtered {
    ...
    return _items.where((it) => matchSearch(it) && matchArea(it) && matchTipe(it)).toList();
  }

  void _onDownload() {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('TODO: Unduh data customer')),
    );
  }

  void _onAdd() {
    Navigator.push(context, MaterialPageRoute(builder: (_) => const TambahCustomerPage()));
  }

  void _onHargaPerCustomer() {
    Navigator.push(context, MaterialPageRoute(builder: (_) => const HargaProdukCustomerPage()));
  }

  void _onEdit(CustomerItem item) {
    Navigator.push(context, MaterialPageRoute(builder: (_) => EditCustomerPage(item: item)));
  }
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          Expanded(
            child: list.isEmpty
                ? const Center(child: Text('Data customer kosong'))
                : ListView.separated(
...
class CustomerItem {
  ...
  final String type; // Reguler/Subdis/PMI
  final String area;
